...
parser.add_argument('--size_val', '-s', type=int, default=1000, help='number of samples in the validation set')
...
general_feat = np.load("%s/general_feat.npy" %args.input_path)
general_label = np.load("%s/general_label.npy" %args.input_path)
general_adjfloat = np.load("%s/general_adjfloat.npy" %args.input_path)
general_adj1 = np.load("%s/general_adj1.npy" %args.input_path)
general_adj2 = np.load("%s/general_adj2.npy" %args.input_path)
general_adj4 = np.load("%s/general_adj4.npy" %args.input_path)
general_adj8 = np.load("%s/general_adj8.npy" %args.input_path)

for i in range(general_label.shape[0]):
    feats.append(general_feat[i])
    labels.append(general_label[i])
    adjs_float[0].append(general_adjfloat[0][i])
    adjs_1[0].append(general_adj1[0][i])
    for num in range(2): adjs_2[num].append(general_adj2[num][i])
    for num in range(4): adjs_4[num].append(general_adj4[num][i])
    for num in range(8): adjs_8[num].append(general_adj8[num][i])
    train_cnt += 1
    name = general_label[i][0]
    affinity = general_label[i][1]
    if not name in docking_dict.keys():
        continue
    cnt = 0
    for y in docking_dict[name]:
        ligand = next(pybel.readfile('pdbqt', "data/docking/%s/%s" %(name, y)))
        pocket = next(pybel.readfile('mol2', "database/general-set-except-refined/%s/%s_pocket.mol2" %(name, name)))
        complex_atoms = get_atoms(ligand, pocket)
        if len(complex_atoms) > max_len:
            print("more atoms then %d (general set)" %max_len)
            continue
        feat, adj_float, adj_1, adj_2, adj_4, adj_8 = graph_featurizer(complex_atoms, max_len)
        feats.append(feat)
        labels.append([name, affinity])
